...
namespace Dolphin2._0.Controllers
{
    ...
    public class TestController : ApiController
    {
        ...
        public HttpResponseMessage ValidateUser(LoginRequest request)
        {
            try
            {
                //var param = JsonConvert.DeserializeObject<LoginRequest>(LoginDetails.ToString());
                return Request.CreateResponse(new { status = true, result = _user.ValidateUser(request) });
            }
            catch(Exception ex)
            {
                //var param = JsonConvert.DeserializeObject<LoginRequest>(LoginDetails.ToString());
        ...
        public HttpResponseMessage GetUserInfo(object UserLogon)
        {
            var param = JsonConvert.DeserializeObject<LoginRequest>(UserLogon.ToString());
            return Request.CreateResponse(new { status = true, result = _user.GetUserInfo(param) });   
        }
        ...
        public HttpResponseMessage GetUserMenu(object UserMenu)
        {
            var param = JsonConvert.DeserializeObject<LoginRequest>(UserMenu.ToString());
            return Request.CreateResponse(new { status = true, result = _menu.GetUserMenu(param.UserName) });
        }
        ...
        public HttpResponseMessage UserLogout(object LoginObj)
        {
            var request = JsonConvert.DeserializeObject<LoginRequest>(LoginObj.ToString());
            return Request.CreateResponse(new { status = true, result = _audit.UserLogout(request) });
        }
        ...
        public HttpResponseMessage ForgotPassword(object LoginObj)
        {
            var request = JsonConvert.DeserializeObject<LoginRequest>(LoginObj.ToString());
            return Request.CreateResponse(new { status = true, result = _user.SendPasswordNotification(request) });
        }
        ...
        public HttpResponseMessage ListClient()
        {
        ...
        {
            var result = Request.CreateResponse(new { status = true, result = _user.ListAvailableUser() });
            return result;
        }
